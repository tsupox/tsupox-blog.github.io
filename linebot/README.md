...
- 📱 LINE経由での対話型ブログ投稿作成
- 🖼️ 画像の自動処理とアップロード（JPEG, PNG, GIF, WebP対応）
- 🏷️ 既存タグの選択と新規タグ作成
- 📝 Hexo形式のMarkdown自動生成
- 🚀 GitHubへの自動コミット・プッシュ
...
- AWSアカウント（Lambda, DynamoDB, S3, API Gateway）
...
# AWS設定（CloudFormationで自動設定される）
DYNAMODB_TABLE_NAME=linebot-blog-publisher-sessions
S3_BUCKET_NAME=linebot-blog-publisher-images-XXXXX
...
3. 指示に従って以下を入力：
   - タイトル（100文字以内）
   - 本文（5000文字以内）
   - 画像（JPEG, PNG, GIF, WebP形式、最大10MB）
   - タグ（既存タグから選択または新規作成）
4. 内容を確認後「はい」で公開
5. 自動的にGitHubにコミットされ、ブログが更新される
...
投稿が公開されると、以下のファイルがGitHubリポジトリに自動コミットされます：

- `source/_posts/YYYYMMDD-{title-slug}.md` - ブログ記事のMarkdownファイル
- `source/images/{year}-rakugaki/{timestamp}-{random}.{ext}` - 投稿画像
